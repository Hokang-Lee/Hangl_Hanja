# -*- coding: utf-8 -*-
# Legacy safety variant. Use convert_hanja.py for normal operation.
"""
...
def _set_sz(rpr, new_val):
    """Set w:sz and w:szCs to new_val in a w:rPr element."""
    for tag in (f"{{{NS}}}sz", f"{{{NS}}}szCs"):
        node = rpr.find(tag)
        if node is not None:
            node.set(f"{{{NS}}}val", new_val)
        else:
            # Create if missing
...
def _ensure_rpr(r):
    """Return existing w:rPr or create one."""
    rpr = r.find(f"{{{NS}}}rPr")
    if rpr is None:
...
def convert_font_size(elem):
    ...
    for para in elem.iter(f"{{{NS}}}p"):
        ...
        # --- Rule 1: 16pt runs containing digits -> 14pt ---
        ...
        # --- Rule 2: Runs inside parentheses -> 14pt ---
        paren_ranges = []
        ...
        pos = 0
        run_positions = []  # (start_pos, end_pos) for each run
        ...
        for paren_start, paren_end in paren_ranges:
            for i, (rstart, rend) in enumerate(run_positions):
                # Run overlaps with paren range
                if rend <= paren_start or rstart >= paren_end:
                    continue
                r = runs[i]
                rpr = _ensure_rpr(r)
                _set_sz(rpr, NEW_SZ)
